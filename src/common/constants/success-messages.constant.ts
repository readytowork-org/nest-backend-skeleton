export const SuccessMessages = {
  // Authentication Success Messages
  REGISTRATION_SUCCESS: 'Registration completed successfully',
  LOGIN_SUCCESS: 'Login successful',
  LOGOUT_SUCCESS: 'Logout successful',
  TOKEN_REFRESH_SUCCESS: 'Token refreshed successfully',

  // Password Management Success Messages
  PASSWORD_RESET_EMAIL_SENT: 'Password reset email has been sent successfully',
  PASSWORD_RESET_SUCCESS: 'Password has been reset successfully',
  PASSWORD_CHANGED_SUCCESS: 'Password changed successfully',

  // User Management Success Messages
  USER_CREATED_SUCCESS: 'User created successfully',
  USER_UPDATED_SUCCESS: 'User updated successfully',
  USER_DELETED_SUCCESS: 'User deleted successfully',
  USER_PROFILE_UPDATED: 'Profile updated successfully',

  // General Success Messages
  OPERATION_SUCCESS: 'Operation completed successfully',
  DATA_RETRIEVED_SUCCESS: 'Data retrieved successfully',
  EMAIL_SENT_SUCCESS: 'Email sent successfully',
  VERIFICATION_SUCCESS: 'Verification completed successfully',

  // Resource Management Success Messages
  RESOURCE_CREATED_SUCCESS: 'Resource created successfully',
  RESOURCE_UPDATED_SUCCESS: 'Resource updated successfully',
  RESOURCE_DELETED_SUCCESS: 'Resource deleted successfully',

  // File Management Success Messages
  FILE_UPLOADED_SUCCESS: 'File uploaded successfully',
  FILE_DELETED_SUCCESS: 'File deleted successfully',


  //otp Success Message
  OTP_CREATE_SUCCESS: 'OTP code send successfully',
  OTP_UPDATE_SUCCESS: 'OTP signup session updated successfully',
  OPT_VERIFIED_SUCCESS: 'OTP has been verified successfully',

  // Generic Success Message
  SUCCESS: 'Success',
};
